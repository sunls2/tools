...
func Checker(opts Options) {
	...
	newList := newPlaylist(opts.M3UFirst, make([]stream, 0, len(availableList.streamList)))
...
func epgXML(ex string, sMap map[string]struct{}, header bool) ([]string, []string, int) {
	data := readSource(ex)
	var channel []string
	...
	for scanner.Scan() {
		...
		if header && len(channel) == 0 && strings.HasPrefix(txt, "<?xml") {
			channel = append(channel, txt)
		}
		...
		if i := strings.Index(txt, channelFlag); i > 0 {
			...
			if _, ok := sMap[channel]; ok {
				programme = append(programme, txt)
			}
		}
	}
...
func checkEpgXML(sList []stream, opts Options) {
	...
	var programme []string
	var count int
	epgXmls := strings.Split(opts.EpgXml, ",")
	for i, v := range epgXmls {
		if len(v) == 0 {
			continue
		}
		ch, p, c := epgXML(v, sMap, i == 0)
		channel = append(channel, ch...)
		programme = append(programme, p...)
		count += c
	}

	programme = append(programme, "</tv>")
	...
	for _, v := range append(channel, programme...) {
		cf.WriteString(v)
		cf.WriteByte('\n')
	}
	cf.Flush()
